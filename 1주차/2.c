...
int function(); // 자신이 호출 된 횟수를 반환하는 함수

int main() {
    int call; // 호출 된 횟수가 저장 될 변수

    for (int i = 0; i < 3; i ++) { // 처음 3번 호출
        call = function();
    }
    printf("현재까지 %d번 호출 됐습니다\n", call); // 호출된 횟수 출력

    for (int i = 0; i < 5; i++) { // 5번 추가 호출
        call = function();
    }
    printf("현재까지 %d번 호출 됐습니다\n", call); // 3번 이후 5번 더 출력 됐기 때문에 8번 출력 확인

    return 0;
}

int function() {
    static int repeat = 0; // 이 함수 내부에서만 변화시킬 수 있고, 프로그램이 종료 될 때 사라지는 정적변수 생성
    repeat++;
