...
void copyarray(double *source, double *target, int size) {
    target = (double*)malloc(sizeof(double)*size); // 복사한 원소를 저장할 메모리를 새로 할당합니다.
    ...
    free(target); // 동적 할당한 주소를 해제합니다.
}

// 처음에는 coppyarray가 동적할당한 주소를 리턴해서, main에서 복사한 내용을 출력하려 했습니다.
// 하지만 그러면 target 배열을 인자로 받을 이유가 없다고 생각해서,
